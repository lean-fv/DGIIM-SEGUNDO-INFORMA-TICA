...
public class GameUniverse {
    ...
    */
    ...
    public void Roulette(){
        ...
        if(currentStation.getRoulette()){
            
            currentStation.setRoulette();
            int player;
            do{

                player = dice.whoStarts(spaceStations.size());

            } while(player==currentStationIndex);

            if (dice.Roulette()){

                Weapon weapon = currentStation.getWeaponInHangar(0);
                currentStation.discardWeaponInHangar(0);
                if (weapon!=null) spaceStations.get(player).receiveWeapon(weapon);

            }

            else{
                Weapon weapon = spaceStations.get(player).getWeaponInHangar(0);
                spaceStations.get(player).discardWeaponInHangar(0);
                if (weapon!=null) currentStation.receiveWeapon(weapon);
