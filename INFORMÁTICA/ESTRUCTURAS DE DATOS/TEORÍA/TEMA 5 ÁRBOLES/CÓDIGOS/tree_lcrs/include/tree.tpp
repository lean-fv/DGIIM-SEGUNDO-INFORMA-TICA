...
template<typename T>
tree<T>::level_iterator::level_iterator() {}

template<typename T>
tree<T>::level_iterator::level_iterator(const tree::node &n) : current(n) {
  if (!n.is_null()){
    this->node_queue.push(n);
  }
}

template<typename T>
bool tree<T>::level_iterator::operator!=(const tree::level_iterator &i) const {
  return !(*this == i);
}

template<typename T>
bool tree<T>::level_iterator::operator==(const tree::level_iterator &i) const {
  return (this->current == i.current && this->node_queue == i.node_queue);
}

template<typename T>
T &tree<T>::level_iterator::operator*() {
  return *this->current;
}
...
tree<T>::level_iterator::operator++() {
  if (!this->current.is_null()){
    if (!this->current.right_sibling().is_null()) {
      this->current = this->current.right_sibling();
      if (!this->current.left_child().is_null()) {
	this->node_queue.push(this->current);
      }
    }
    else {
      if (this->node_queue.size() > 0) {
	typename tree<T>::node next = this->node_queue.front();
	this->node_queue.pop();
	this->current = next.left_child();
	if (!this->current.is_null() && !this->current.left_child().is_null()) {
	  this->node_queue.push(this->current);
	}
      }
      else {
	this->current = typename tree<T>::node();
      }
    }
  }
  return *this;
...
template<typename T>
typename tree<T>::level_iterator tree<T>::begin_level() {
  return tree::level_iterator(this->root);
}

template<typename T>
typename tree<T>::level_iterator tree<T>::end_level() {
  return tree::level_iterator(node());
}

template<typename T>
tree<T>::const_level_iterator::const_level_iterator() {}

template<typename T>
tree<T>::const_level_iterator::const_level_iterator(const tree::node &n) : current(n) {
  if (!n.is_null()){
    this->node_queue.push(n);
  }
}
...
template<typename T>
bool tree<T>::const_level_iterator::operator!=(const tree::const_level_iterator &i) const {
  return !(*this == i);
}
...
template<typename T>
const T &tree<T>::const_level_iterator::operator*() const {
  return *this->current;
}
...
template<typename T>
typename tree<T>::const_level_iterator tree<T>::cbegin_level() const {
  return tree::const_level_iterator(this->root);
}

template<typename T>
typename tree<T>::const_level_iterator tree<T>::cend_level() const {
  return tree::const_level_iterator(node());
}
